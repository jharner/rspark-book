...
```
We do this in order to have the same data partitions as in Section 10.2.
...
coef(slump_glmfit)
```
The regression coefficient estimates from `glmnet` are similar to, but equal to the coefficient estimates from `sparklyr`'s `ml_linear_regression`. This is due to differences in the package algorithms underlying linear regression.
...
```{r}
slump_glm_lasso_cv <- cv.glmnet(x = slump.x, y = slump.y, type.measure = "mse", alpha = 1,
                                nfolds = 10)
slump_glm_lasso_cv$lambda.min
```
Our choice of $\lambda$ based on 10-fold cross-validation is less than what we choose in Section 10.2.

The coefficient estimates, based on 10-fold cross-validation, are:
```{r}
coef(slump_glm_lasso_cv, s = "lambda.min")
```
Notice that `cement`, `fly_ash`, and `water` (and possibly coarse_aggr`) converge more slowly to 0, which agrees with our results in Sections 7.1 and 10.2.
...
```{r}
slump_glm_enet_cv <- cv.glmnet(x = slump.x, y = slump.y, type.measure = "mse", alpha = 0.5,
                               nfolds = 10)
...
It would be possible to build a parameter grid extending the idea in Section 7.1. We could assess performance metrics over this grid and then test. For now you can experiment with different values of $\alpha$ and $\lambda$ or minimize $\lambda$ for various $\alpha$ directly with `glmnet`.
