...
$$
  e_i = y_i - \hat{y}_i,
$$
where $\hat{y}_i = \hat\beta_0 + \hat\beta_1 x_{i1} + \cdots + \hat\beta_p x_{ip}$ is the predicted value for the $i^\mbox{th}$ observation.
...
$$
i.e., the coefficient for the $j^\mbox{th}$ predictor is 0. In order to test this hypothesis, we compute a $t$-test as follows:
$$
...
### 6.1.2 Concrete Slump Test Data
...
slump_sdf <- spark_read_csv(sc, "slump_sdf",
                    path =  "file:///home/rstudio/rspark-notes/data/slump.csv")
head(slump_sdf)
```
