...
Performance metrics for regression are now obtained by getting predictions using the test data based on the full model and then using the `ml_regression_evaluator` to get specific metrics.
```{r}
slump_lr_full_predict <- ml_predict(slump_lr_full_model, slump_test_sdf)
slump_lr_metrics <-
  data.frame(lambda = 0,
...
```{r}
regParm <- c(0.005, 0.01, 0.02, 0.04, 0.06, 0.08, 0.1)
for(l in regParm) {
  slump_lr_fit <- slump_lr_model(l) 
  slump_lr_predict <- ml_predict(slump_lr_fit, slump_test_sdf)
  slump_lr_metrics <- 
    data.frame(lambda = l,
