...
```{r}
# write.csv(nemotemp, file="nemotemp.csv", quote=F, row.names=F)
dir <- getwd()
write_csv(nemotemp2, file.path(dir, "nemotemp2.csv"))
```
...
```{r}
# nemotempcsv <- read.csv("nemotemp.csv", header = T)
nemotemp2csv <- read_csv("nemotemp2.csv")
nemotemp2csv
```
Here the column types were determined by default.
