...
nemotemp_tbl
```
The result is a `POSIXct` vector with a time zone attribute. In the `format` argument, `%d` is a 2-digit specification for day, `%b` is the abbreviated name for month, and `%Y%` is the 4-digit representation for year. At this point we can do arithmetical operations on `date`. For a complete list of formating options for dates (and times) see the documentaion for `parse-datetime`.
...
Underlying the printed representation of a date is a numeric value equal to the number of days since Jan. 1, 1970 (a UNIX convertion).
```{r}
