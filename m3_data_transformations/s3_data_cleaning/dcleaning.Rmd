...
Most *statistical datasets* can be represented in data frames made up of rows and columns. The columns are almost always labeled and the rows are sometimes labeled. The following code provides data in a format commonly seen.
```{r}
preg <- read.csv("preg.csv", stringsAsFactors = FALSE)
preg
```

The `preg` data  is not tidy! The `treatment` values (`a` and `b`) are the column names, i.e., the columns are not variables.
...
In popular parlance `gather` makes wide data long and `spread` makes long data wide.

The `gather` verb can make a tidy data frame:
```{r}
preg_df <- preg %>% 
...
How is this data represented, e.g., the repeated measurements over days for each person? Statisticians constructing data frames in R face the same issues as those faced by database designers.
...
Hadley provides a series of examples in his vignette. We will focus an a single example which illustrates gathering and spreading and the use of pipes to build a tidy workflow.
...
It has variables in individual columns (`id`, `year`, `month`), spread across columns (`d1-d31`, i.e., days) and across rows (`tmin`, `tmax`, i.e., minimum and maximum temperature). Months with fewer than 31 days have structural missing values for the last day(s) of the month.
...
```
This example shows the power of `tidyr` together with `dplyr` in cleaning data. Note, we could reconstruct the non-structural missing values since we know how many days are in each month.
