...
Linux manages the communication between your software and your hardware resources. This course uses Docker containers based on Linux. For a more complete reference see: [What is Linux?](https://www.linux.com/what-is-linux).

Linux OS has a number of components, including:  

* the kernel for managing the CPU, memory, devices, etc.;   
* daemons for running background processes at start up or those initiated by the user;  
* the shell for issuing commands (the bash shell in our case).  
...
The permission string is given by the first 10 characters in each output line from `ls -al`. After the first character, we have permissions (file/directory modes) in three sequences of three characters each. For example, for `rspark-notes`: 
...
`rspark` can be installed by pulling the Docker images from Docker Hub. We will discuss `git` in Section 1.4, but for now run the following command in the terminal:  
```
...
./start.sh
```
The last command runs the `start.sh` shell script and builds the containers. You must have execute privileges to run the shell script. If not, use `chmod`, e.g., `chmod 755 start.sh`.
...
Note: the build option is only specified the first time or after you have issued:  
```
git pull origin master
```
to update you local repo.

Unless you have just upgraded `rspark`, simply start the containers by:
```
./start.sh
```
...
You can enter containers from the local command line using `ssh` (secure shell). For example, to `ssh` into the `rstudio` container run the following in your local terminal:
```
docker exec -ti rspark-docker_rstudio_1 bash
cd home/rstudio
```
where "rspark-docker_rstudio_1" is the container `NAME`. 
...
Note that the `Terminal` tab within RStudio's `Console` panel is running bash within the `rstudio` container. Ironically, you can run a separate R process within the Terminal.
...
```
If you are having problems with your containers, you can stop and then delete the contanmers and images (in this order). Once you delete the containers, images, and perhaps the volumes, then you must download new images from Docker Hub or rebuild the images from GitHub. 

### 1.3.4 Vagrant/ VirtualBox Virtualization

If you cannot install `rspark` using Docker, then try Vagrant/ VirtualBox. Vagrant is heavier than Docker and the download is longer, but it works well.
...
Windows users will need to install [git for Windows](https://git-for-windows.github.io) for bash access (as above) and get the 64-bit version of Vagrant.
...
```
If `rspark-vagrant` gets updated (I will notify you if this is the case.), execute:
```
git pull origin master
```
