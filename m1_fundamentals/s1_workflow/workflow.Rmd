...
It focuses on `tidyr` to make data tidy, `dplyr` for transforming the data, `ggplot2` for visualization, various modeling functions such as `lm`, and R markdown or Shiny for communication. We will often be using his approach in this course.  
...
The trend is towards deploying software using [Docker containers](https://www.docker.com/). Docker allows applications and their dependencies to run in virtual containers using Linux as the base OS. An excellent repository exists for Dockerized versions of R called [rocker](http://github.com/rocker-org/rocker). We use the Docker image `verse` which adds the `rstudio`, `tidyverse`, and `verse` layers to the `r-ver` image, which in our deployment is based on R 3.4.4.

We built `rspark` to extend `rocker` to the Spark ecosystem. The GitHub repo for `rspark` can be found [here](http://github.com/jharner/rspark). `rspark` consists of containers for RStudio, PostgreSQL, Hadoop, Hive, and Spark. By default, Spark has a master and two workers although it can also be run within the RStudio container as an option. The latter approach has a smaller footprint and is often adequate for teaching.
