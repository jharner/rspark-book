...
### 1.2.1 Running RStudio Server

R/RStudio is run in a Docker container, which is part of a virtual computing environment called `rspark`.

We run RStudio Server in a virtual cluster on your local machine. The development repository (repo) for the virtual cluster is called `rspark` and can be found on GitHub here:   [github.com/jharner/rspark](https://github.com/jharner/rspark). This virtual cluster will allow you to run Spark, Hadoop, etc.
...
Detailed instruction for running `rspark` an AWS are available here: [rspark on AWS](https://github.com/jharner/rspark/tree/master/AWS)

Note: The pre-built image on AWS is called `rsparkbox` and typically it is a longer version of this tutorial.

Once you follow the steps in the directions, connect to your `rspark` server through a web browser:

Into a browser's URL bar, enter 'http://0.0.0.0:8787' replacing '0.0.0.0' with the IP address of your instance. Log into RStudio with the credentials:
...
#### Dispose of the `rspark` Workspace
...
R supports Projects which allow related files to be organized together. Projects can then be tied to version control systems such as `git`. We will be using `rspark-notes` in this course, optionally as an RStudio Project.

### 1.2.3 The Console Panel
...
The `Terminal` tab provides access to the Linux command line running `bash`. By default, you open a session in `Terminal 1`. Your working directory is displayed to the right of the terminal name, which can be changed. The prompt is given by `rstudio@CONTAINER_ID`. Additional terminals can be launched by selecting `Terminal` and then `New Terminal` in the `Tools` menu. Each terminal can be set to a specific directory for executing commands.

### 1.3.3 Environments and Files

R has a workspace in which all assigned data structures or functions are kept. These structures or functions show up in the `Environment` tab. The files and directories are available in the `Files` tab. Spark data structures appear in the `Connections` tab.
