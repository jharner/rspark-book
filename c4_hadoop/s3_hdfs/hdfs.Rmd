...
Hadoop Distributed File System (HDFS) is Hadoop’s main filesystem, but Hadoop has a general-purpose filesystem abstraction. Thus, Hadoop integrates with other storage systems, e.g., the local filesystem and Amazon's S3.

### 4.3.1 HDFS Design

HDFS is designed for storing very large files with streaming data access, running on clusters of commodity hardware.  

* Very large files: Files are often stroed in a [data lake](https://en.wikipedia.org/wiki/Data_lake) in raw format or as blobs. Data lakes are often terabytes, or even petabytes, in size.  
